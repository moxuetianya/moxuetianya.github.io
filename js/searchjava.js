    ...
    if (document.body.contains(document.goSearch)) {
      ...
      */
      (function(){
        ...
        // redirect to search page when press enter or click search icon
        function goSearchNow() {
            var value = document.getElementById("searchinput").value.trim();
            if (value) {
                window.location.href = document.querySelector("meta[name='base']").getAttribute("content") + "/search/?q=" + encodeURIComponent(value);
            }
            return false
        }
